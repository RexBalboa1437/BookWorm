   ...
   if (mysqli_num_rows($select_product_name) > 0) {
      ...
      if ($add_product_query) {
         if ($image_size > 2000000) {
            ...
            mysqli_query($conn, "UPDATE `products` SET image = '$image_path' WHERE id = '$product_id'") or die('query failed');
         }
      } else {
...
if (isset($_GET['delete'])) {
   ...
   $image_to_delete = $fetch_delete_image['image'];
...
if (isset($_POST['update_product'])) {
   ...
   $update_image_path = $update_folder . $update_image;
   ...
   if (!empty($update_image)) {
      if ($update_image_size > 2000000) {
         ...
         mysqli_query($conn, "UPDATE `products` SET image = '$update_image_path' WHERE id = '$update_p_id'") or die('query failed');
      }
   }
